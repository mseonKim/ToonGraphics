...
half LinearStep(float x, float M, float m)
{
    ...
    half clearCoatMask, bool specularHighlightsOff)
{
    ...
    half hLambert = NdotL * 0.5 + 0.5;
    half shadowMask = LinearStep(hLambert, _BaseColorStep + _BaseColorStepFeather, _BaseColorStep - _BaseColorStepFeather);

    // half3 radiance = lightColor * (lightAttenuation * NdotL);
    half3 radiance = lightColor * (lightAttenuation * hLambert);

    // half3 brdf = brdfData.diffuse;
    half3 brdf = lerp(0, brdfData.diffuse, shadowMask);
#ifndef _SPECULARHIGHLIGHTS_OFF
    [branch] if (!specularHighlightsOff)
    {
        // brdf += brdfData.specular * DirectBRDFSpecular(brdfData, normalWS, lightDirectionWS, viewDirectionWS);
        brdf += _SpecIntensity * LinearStep(brdfData.specular * DirectBRDFSpecular(brdfData, normalWS, lightDirectionWS, viewDirectionWS), _SpecStep + _SpecStepFeather, _SpecStep - _SpecStepFeather);
